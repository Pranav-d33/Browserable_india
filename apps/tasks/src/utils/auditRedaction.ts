...
export function redactSensitiveString(value: string): string {
  ...
  const hasSensitivePattern = SENSITIVE_PATTERNS.some(pattern => pattern.test(value));
 ...
 */
export function redactAuditLog(auditLog: Record<string, unknown>): Record<string, unknown> {
 ...
 */
export function redactAuditLogs(auditLogs: Record<string, unknown>[]): Record<string, unknown>[] {
...
export function containsSensitiveData(value: unknown): boolean {
  ...
  if (typeof value === 'object' && value !== null) {
    ...
    for (const [key, val] of Object.entries(value)) {
      if (SENSITIVE_PATTERNS.some(pattern => pattern.test(key)) || containsSensitiveData(val)) {
        return true;
      }
    }
  }
