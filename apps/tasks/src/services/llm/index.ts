...
class LLMFactory {
  ...
  private initializeProviders(): void {
    ...
    if (envProvider && this.providers.has(envProvider)) {
      ...
      logger.info(
        ...
        'Set default LLM provider from environment'
      );
    }
    ...
    if (envModel) {
      ...
      logger.info(
        { model: envModel },
        'Set default LLM model from environment'
      );
    }
  ...
  async healthCheck(): Promise<
    Record<string, { status: 'healthy' | 'unhealthy'; error?: string }>
  > {
  ...
  getProviderStats(): Record<string, { circuitBreaker?: unknown }> {
    const stats: Record<string, { circuitBreaker?: unknown }> = {};
