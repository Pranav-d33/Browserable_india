...
const envSchema = z.object({
  ...
  AGENT_RUN_TIMEOUT_MS: z
    ...
    .transform(val => parseInt(val, 10))
    .default('300000'),
  MAX_LLM_CALLS_PER_RUN: z
    .string()
    .transform(val => parseInt(val, 10))
    .default('10'),
  MAX_BROWSER_STEPS_PER_RUN: z
    .string()
    .transform(val => parseInt(val, 10))
    .default('50'),
  AGENT_QUEUE_CONCURRENCY: z
    .string()
    .transform(val => parseInt(val, 10))
    .default('5'),
  BROWSER_QUEUE_CONCURRENCY: z
    .string()
    .transform(val => parseInt(val, 10))
    .default('2'),

  // Rate limiting configuration
  USER_RATE_LIMIT_PER_MINUTE: z
    .string()
    .transform(val => parseInt(val, 10))
    .default('120'),
  USER_MAX_CONCURRENT_RUNS: z
    .string()
    .transform(val => parseInt(val, 10))
    .default('3'),
...
function validateEnvironment() {
  ...
  try {
    ...
    if (!isDev && !isTest) {
      console.log(
        '🚨 Production environment detected - enforcing strict validation'
      );

      // Ensure all required production variables are set
      const requiredForProd = ['JWT_SECRET'];
      ...
      if (
        !env.USE_LOCAL_STORAGE &&
        (!env.S3_ENDPOINT || !env.S3_ACCESS_KEY || !env.S3_SECRET_KEY)
      ) {
        throw new Error(
          'Either USE_LOCAL_STORAGE=true or S3 configuration must be complete'
        );
      }
    ...
    console.error(
      '   USE_SQLITE=true - Use local SQLite database (recommended for dev)'
    );
    console.error(
    ...
    console.error(
      '   USE_LOCAL_REDIS=true - Use local Redis (recommended for dev)'
    );
    console.error('   REDIS_URL - Redis connection string (for production)');
    console.error('\n📋 Storage configuration (choose one):');
    console.error(
      '   USE_LOCAL_STORAGE=true - Use local file storage (recommended for dev)'
    );
    console.error(
      '   S3_ENDPOINT, S3_ACCESS_KEY, S3_SECRET_KEY - S3 configuration (for production)'
    );
    console.error('\n📋 LLM configuration:');
    console.error(
      '   GEMINI_API_KEY - Google Gemini API key (recommended for dev)'
    );
