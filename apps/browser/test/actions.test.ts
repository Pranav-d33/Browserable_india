...
describe('BrowserActions', () => {
  ...
  describe('Action Validation', () => {
    it('should validate goto action', async () => {
      const sessionId = await sessionManager.create({ browserType: 'chromium' });
    ...
    it('should validate click action', async () => {
      ...
      expect(() => ClickActionSchema.parse({
        sessionId,
        selector: 'button'
      })).not.toThrow();
    ...
    it('should validate waitFor action', async () => {
      ...
      // Valid selector should pass validation
      expect(() => WaitForActionSchema.parse({
        sessionId,
        target: 'button'
      })).not.toThrow();
    ...
    it('should validate select action', async () => {
      ...
      // Valid action should pass validation
      expect(() => SelectActionSchema.parse({
        sessionId,
        selector: 'select',
        value: 'option1'
      })).not.toThrow();
    ...
    it('should validate evaluate action', async () => {
      ...
      // Evaluate disabled
      await expect(browserActions.evaluate({
        sessionId,
        script: 'document.title'
      })).rejects.toThrow('JavaScript evaluation is disabled');

      // Create actions with evaluate enabled
      const actionsWithEvaluate = new BrowserActions(sessionManager, 30000, true);

      // Safe script
      await expect(actionsWithEvaluate.evaluate({
        sessionId,
        script: 'document.title'
    ...
    it('should validate screenshot action', async () => {
      ...
      // Valid action
      await expect(browserActions.screenshot({
        sessionId,
        fullPage: true
      })).resolves.not.toThrow();
    ...
    it('should validate pdf action', async () => {
      ...
      await expect(browserActions.pdf({
        sessionId
      })).resolves.not.toThrow();

      // Create firefox session
      const firefoxSessionId = await sessionManager.create({ browserType: 'firefox' });
  ...
  describe('Session Management', () => {
    it('should throw error for non-existent session', async () => {
      await expect(browserActions.goto({
  ...
  describe('Error Handling', () => {
    ...
    it('should handle element not found errors', async () => {
      ...
      await expect(browserActions.click({
